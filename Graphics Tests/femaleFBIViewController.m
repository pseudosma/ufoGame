...
@interface femaleFBIViewController ()

@end

@implementation femaleFBIViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        femaleFBIGlassesView *shades = [[femaleFBIGlassesView alloc] initWithFrame:CGRectMake(-4.5, -6, 27, 23)];
        [head addSubview:shades];
        
        skirt = [[femaleFBISkirtView alloc] initWithFrame:CGRectMake(-5, 14, 26, 17)];
        [body addSubview:skirt];
        
        femaleFBIJacketView *jacket = [[femaleFBIJacketView alloc] initWithFrame:CGRectMake(-2, 0, 19, 20)];
        [body addSubview:jacket];

        femaleFBIJacketSleeveView *rSleeve = [[femaleFBIJacketSleeveView alloc] initWithFrame:CGRectMake(-2, -12, 9, 20)];
        rSleeve.layer.anchorPoint = CGPointMake(.5, 0);
        [rArm insertSubview:rSleeve belowSubview:rHand];
        
        femaleFBIJacketSleeveView *lSleeve = [[femaleFBIJacketSleeveView alloc] initWithFrame:CGRectMake(-2, -12, 9, 20)];
...
- (void)recover{
    ...
    [UIView animateWithDuration:.1 delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
        skirt.transform = CGAffineTransformConcat(CGAffineTransformMakeRotation(.5), CGAffineTransformMakeTranslation(-2, -3));
    }completion:^(BOOL finished) {
        [UIView animateWithDuration:.9 delay:0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionBeginFromCurrentState animations:^{
            skirt.transform = CGAffineTransformIdentity;
        } completion:^(BOOL finished) {}];}];
